...
object project extends App{
  ...
  def Task1_2(): Unit = {
    val filename = dataFile.filename
    println("What renewable source would you like to remove or add?")
    val source = scala.io.StdIn.readLine()
    println("How many would you like to remove or add? Please add a minus if you want to remove (eg: -3)")
    ...
    try {
      ...
      for (line <- lines) {
        ...
        println("Do you want to continue for this source? (yes/no)")
        val continue = scala.io.StdIn.readLine().trim.toLowerCase
        if (continue != "yes") {
          println("Stopping data entry for this source.")
          break
        }
      }
  ...
  def Task2():Unit= {
    val filename = dataFile.filename
    val valuefile = new File(dataFile.value)
...
object Task4 {
  ...
  def OrderAlgo(caselist: List[energy_sources]): List[energy_sources] = {
    def get_input(): Option[Int] = {
      println("Please choose an option by which the data should be sorted: \n1 - Hourly\n2 - Daily\n3 - Weekly\n4 - Monthly\nChoose by inserting an integer from 1-4:")
  ...
  def data_analysis(caselist: List[energy_sources]): Unit = {
    def get_input(): Option[Int] = {
      println("What type of data analysis would you like? \n1) Mean\n2) Median\n3) Mode\n4) Range\n5) Midrange\nPlease enter an integer number from 1-5: ")
      val input = scala.io.StdIn.readLine()
      Try(input.toInt) match {
        case Success(x) => {
          if (x > 0 && x < 6) Some(x)
